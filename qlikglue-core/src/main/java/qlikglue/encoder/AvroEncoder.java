 ...
 * for publishing.
 */
public class AvroEncoder implements QlikGlueEncoder {
     ...
     * 
     * @param avroSchema the QlikGlue AvroSchema we are working with.
     ...
     */
    protected GenericRecord populateAvroRecord(AvroSchema avroSchema, DownstreamOperation op, Schema schema) {
        ...
        for (DownstreamColumnData col : op.getColumns()) {
            ...
            column = avroSchema.getAvroColumn(columnName);
     ...
     * @param avroSchema the internal representation of the Avro schema
     * @return the serialized record
     * @throws IOException if there is a problem
     */
    private byte[] serializeRecord(GenericRecord record, Schema schema,
                                   @SuppressWarnings("unused") AvroSchema avroSchema) throws IOException {
        byte[] rval;
     ...
     */
   // public int registerSchema(String schemaName, Schema schema, AvroSchema avroSchema) {
   //     int id = 0;
   //     if (avroSchema.getSchemaId() == AvroSchema.SCHEMA_ID_UNSET)
    //        avroSchema.setSchemaId(id);
    //    return id;
    //}
    ...
    public EventData encodeDatabaseOperation(DownstreamOperation op) throws IOException {
        ...
        AvroSchema avroSchema = AvroSchemaFactory.getAvroSchemaFactory().getAvroSchema(key);
        Schema schema = avroSchema.getAvroSchema();
        
        //registerSchema(key, schema, avroSchema);
        

        eventHeader.setHeaderInfo(op);

        GenericRecord record = populateAvroRecord(avroSchema, op, schema); 

        // find schema
        //Schema schema = getSchema(op);

        rval = serializeRecord(record, schema, avroSchema);
        //out.close(); // noop in the Apache version, so not bothering
