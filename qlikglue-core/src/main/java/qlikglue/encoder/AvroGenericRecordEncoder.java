...
public class AvroGenericRecordEncoder extends AvroEncoder {
    ...
    public EventData encodeDatabaseOperation(DownstreamOperation op) throws IOException {
        ...
        AvroSchema avroSchema = AvroSchemaFactory.getAvroSchemaFactory().getAvroSchema(schemaName);
        Schema schema = avroSchema.getAvroSchema();
        
        super.eventHeader.setHeaderInfo(op);

        GenericRecord record = super.populateAvroRecord(avroSchema, op, schema); 
